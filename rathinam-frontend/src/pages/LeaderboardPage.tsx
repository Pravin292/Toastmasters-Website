...
export const LeaderboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchLeaderboardData = async () => {
      ...
      try {
        ...
        const res = await rankingApi.getCurrentMonthlyChampionship().catch(async () => {
          return await rankingApi.getMonthlyRankings(year, month);
        });
  ...
  if (loading) {
    return (
      <AppLayout title="Leaderboard & Championship">
        <LoadingSpinner />
      </AppLayout>
    );
  }
    ...
    <AppLayout title="Leaderboard & Monthly Championship">
      ...
      {champion ? (
        <div style={{
          ...
          <div>
            <div style={{ fontSize: '0.85rem', textTransform: 'uppercase', letterSpacing: '0.05em', color: '#F2DF74', fontWeight: 700 }}>
              Current Monthly Champion ({champion.year}-{String(champion.month).padStart(2, '0')})
            </div>
        ...
        <div style={{
          ...
          <div>
            <div style={{ fontWeight: 700, color: '#F2DF74' }}>No Monthly Champion Declared Yet</div>
      ...
      <Card>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 18, flexWrap: 'wrap', gap: 12 }}>
          <h3 style={{ fontSize: '1.2rem', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span>Monthly Points Leaderboard</span>
          </h3>
              ...
              {rankingList.length === 0 && (
                <tr>
                  <td colSpan={5} style={{ textAlign: 'center', color: '#64748B', padding: 28 }}>
                    No points recorded for this month yet. Standings will update automatically as attendance and meeting roles are assigned.
                  </td>
                </tr>
              )}
            </tbody>
